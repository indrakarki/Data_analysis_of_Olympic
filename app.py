...
if user_menu == 'Overall Analysis':
    ...
    fig = px.line(nations_over_time,
                  x="Edition", y="region")
    st.plotly_chart(fig)
    st.title("Events Nations Over Time")
    ...
    st.title("Athletes Nations Over Time")
...
if user_menu == 'Country-wise analysis':
    ...
    st.title(selected_country + ' is good in these sports')
    pt = helper.country_event_heatmap(df,selected_country)
    fig, ax = plt.subplots(figsize=(20, 20))
    ax=sns.heatmap(pt,annot=True)
    st.pyplot(fig)

    st.title("Top 10 Most Successfull Athelete of "+selected_country)


    x = helper.most_successfull(df, selected_country)
    st.table(x)
